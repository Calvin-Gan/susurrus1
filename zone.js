...
effect(e) {
  ...
  if (e == 1 && this.opacity > 0) {
    let xc = random(this.position.x - this.range, this.position.x + this.range);
  ...
  let dir = p5.Vector.sub(this.position, target.position);
    ...
    if (distance < this.range) {
      ...
      tangent.setMag(sqrt((abs(5) * 100) / distance));
  ...
  let snowf = createVector(random(-50,-10),random(-2,2))
   ...
   target.applyForce(snowf)
    ...
    //target.velocity.mult(createVector(8,10))
  target.sn = false
  }
  ...
  detect(e) {
    ...
    if (this.visible == true) {
      noFill();
      stroke(108,128,148);
      rect(this.position.x-this.range, this.position.y,this.position.x + this.range,height );
    if(this.opacity == 0)
    {
       zones.splice(this, 1);
    }
